...
if (games.length === 0) {
  console.log(`${ymd}: no women's college basketball games today; leaving snapshot unchanged`);
  process.exit(0);
}
mkdirSync("data", { recursive: true });
writeFileSync(
  "data/latest.json",
  JSON.stringify({ sport: "basketball", league: "womens-college-basketball", date: ymd, count: games.length, games }, null, 2) + "\n",
);
console.log(`${ymd}: wrote ${games.length} women's college basketball games -> data/latest.json`);
