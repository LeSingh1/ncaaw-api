...
const WEB  = "https://site.web.api.espn.com/apis/common/v3/sports/basketball/womens-college-basketball";
...
async function get(url) {
  const res = await fetch(url, { headers: { "user-agent": "ncaaw-api (github.com/LeSingh1/ncaaw-api)" } });
  if (!res.ok) throw new Error(`ESPN ${res.status} for ${url}`);
  return res.json();
}

// women's college basketball is basketball/womens-college-basketball on ESPN.
